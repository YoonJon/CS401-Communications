...
3.1.1.5 All persistent data shall be stored in server-managed files on the Server.

3.1.1.6 Conversation identifiers shall use unique numeric identifiers generated by the Server; user identifiers shall be unique organization-assigned strings.
...
3.1.3.15 The Server shall persistently store all users, conversations, messages, participant changes, and read-status data in server-managed files.
...
3.1.3.18 The Server should avoid duplicate private conversations between the same two users where practical.
...
3.2.4 The system shall interface with the host operating system's file system to persist user data, conversation records, and message logs in server-managed files.
...
4.1.3 The system shall securely store user passwords (hashed and salted) in production deployments.
...
- frame: JFrame
- cards: ScreenCards
- userIdlePollTimer: Timer
- lastUserActivityMillis: long   // volatile; updated by global AWT key/mouse listener
...
+ getConversationViewModel(): DefaultListModel<Message>
+ getConversationListViewModel(): DefaultListModel<Conversation>
+ getAdminConversationSearchWindowModel(): DefaultListModel<ConversationMetadata>
+ isSelectingUsers(): bool
+ isAdminSearchingConversation(): bool

- onUserIdlePollTick(): void   // EDT: compares idle time to threshold; may call controller.logout()
- clearLoginAndRegisterFields(): void   // resets login/register text fields before card switches
...
- requestQueue: LinkedBlockingDeque<Request>
- requestDrainThread: Thread
...
+ register(userId: String, realName: String, loginName: String, password: char[]): void
+ login(loginName: String, password: char[]): void
+ logout(): void
...
+ setCurrentConversationId(conversationId: long): void
+ getCurrentConversationId(): long
+ getCurrentConversation(): Conversation

- disconnectSocket(): void   // synchronized; best-effort LOGOUT on wire, then close streams/socket
...
- handleLoginResultResponse(response: Response): void
...
- responseQueue: LinkedBlockingQueue<Map.Entry<String, Response>>
...
- usersByUserID: ConcurrentMap<String, User>
- usersByLoginName: ConcurrentMap<String, User>
- conversationIDsByUserID: ConcurrentMap<String, Set<Long>>
- userIDsByConversationID: ConcurrentMap<Long, Set<String>>
- conversationsByConversationID: ConcurrentMap<Long, Conversation>
- authorizedUsers: Map<String, String>
- authorizedAdminIds: ArrayList<String>
- directoryUserInfos: CopyOnWriteArrayList<UserInfo>
- dirtyUsers: Set<String>
- dirtyConversations: Set<Long>
- dataRoot: File  // and resolved subpaths: server_data/, conversation_data/, user_data/, etc.
- writerThread: Thread

+ DataManager(dataRootPath: String)
+ close(): void
+ writeDirty(): void   // synchronous flush; throws IOException on I/O failure
+ handleRegister(request: Request): Response
+ handleLogin(request: Request): Response
+ handleSendMessage(request: Request): Response
+ handleUpdateReadMessages(request: Request): Response
+ handleCreateConversation(request: Request): Response
...
- writerLoop(): void   // background writer thread body; periodic writeDirty()
static nextMessageSequenceNumber(): long   // package-private static; AtomicLong increment
static nextConversationId(): long   // package-private static; AtomicLong increment
- persistUser(user: User): void   // mark user id dirty
- persistConversation(conversation: Conversation): void   // mark conversation id dirty
- userPersistenceFile(userId: String): File
- conversationPersistenceFile(conversationId: long): File
- loadServerCounters(): void   // throws IOException
- persistServerCounters(): void   // throws IOException
- newConcurrentStringSet(): Set<String>
- newConcurrentLongSet(): Set<Long>
- isValidUser(userID: String, userName: String): boolean   // authorized CSV lookup
- loadData(): void   // startup rehydration + directory build
- loadAuthorizedUsersCsv(file: File): void   // throws IOException
- loadAuthorizedAdminsCsv(file: File): void   // throws IOException
- buildDirectoryAtStartup(): void
- addUserToDirectory(user: User): void   // sorted insert into directoryUserInfos
- getDirectorySnapshot(): ArrayList<UserInfo>
- updateConversation(message: Message): void   // append in memory + persistConversation
- linkUserToConversation(userId: String, conversationId: long): void
- linkParticipantsToConversation(conversationId: long, participants: Iterable<UserInfo>): void
- unlinkUserFromConversation(userId: String, conversationId: long): void
static containsParticipant(participants: ArrayList<UserInfo>, userId: String): boolean


class User   // shared.networking.User — server-persisted account
...
- password: String
- userType: UserType
- lastRead: Map<Long, Long>   // conversationId → last seen message sequence

+ User(id: String, n: String, ln: String, p: String, userType: UserType)
+ getUserId(): String
+ getName(): String
+ getLoginName(): String
+ getPassword(): String
...
+ toUserInfo(): UserInfo
+ getLastRead(conversationId: long): long
+ setLastRead(conversationId: long, sequenceNumber: long): void
+ removeConversationFromLastReadMap(conversationId: long): void
+ static fromFile(f: File): User

  <<nested>> class UserInfo implements Serializable
  - userId: String
  - name: String
  - userType: UserType
  - lastRead: Map<Long, Long>
  + getName(): String
  + getUserId(): String
  + getUserType(): UserType
  + getLastReadMap(): Map<Long, Long>
  + getLastRead(conversationId: long): long
  + setLastRead(conversationId: long, sequenceNumber: long): void
```
...
+ isAuthenticated(): boolean
  ...
  + run(): void
  + RequestListener()
  - handleSessionTransition(request: Request, response: Response): void   // private; LOGIN success → addSession; LOGOUT → removeSession
...
+ Request(t: RequestType, senderId: String)
+ Request(t: RequestType, p: RequestPayload, senderId: String)
+ getType(): RequestType
...
- type: ResponseType
- payload: ResponsePayload   // may be null when not applicable

+ Response(t: ResponseType)
+ Response(t: ResponseType, p: ResponsePayload)
...
```
class RawMessage implements RequestPayload
- text: String
- targetConversationId: long
+ RawMessage(t: String, targetConversationId: long)
+ getText(): String
+ getTargetConversationId(): long

class UpdateReadMessages implements RequestPayload
- conversationID: long
- lastSeenSequenceNumber: long
+ UpdateReadMessages(conversationID: long, lastSeenSequenceNumber: long)
+ getConversationID(): long
+ getLastSeenSequenceNumber(): long

class LoginCredentials implements RequestPayload
- loginName: String
- password: String
...
class RegisterCredentials implements RequestPayload
- userId: String
- loginName: String
- password: String
- name: String
...
class CreateConversationPayload implements RequestPayload
- participants: ArrayList<UserInfo>
+ CreateConversationPayload(p: ArrayList<UserInfo>)
+ getParticipants(): ArrayList<UserInfo>

class AddToConversationPayload implements RequestPayload
- participants: ArrayList<UserInfo>
- targetConversationId: long
+ AddToConversationPayload(p: ArrayList<UserInfo>, targetConversationId: long)
+ getParticipants(): ArrayList<UserInfo>
+ getTargetConversationId(): long

class LeaveConversationPayload implements RequestPayload
- targetConversationId: long
+ LeaveConversationPayload(targetConversationId: long)
+ getTargetConversationId(): long

class JoinConversationPayload implements RequestPayload
- targetConversationId: long
+ JoinConversationPayload(targetConversationId: long)
+ getTargetConversationId(): long

class AdminConversationQuery implements RequestPayload
- userId: String
...
```
class Conversation implements ResponsePayload
- conversationId: long
...
+ addParticipants(toAdd: ArrayList<UserInfo>): void
+ removeParticipant(userId: String): void
+ static fromFile(f: File): Conversation
+ append(m: Message): void

class ConversationMetadata implements ResponsePayload
- conversationId: long
- participants: ArrayList<UserInfo>
- historicalParticipants: ArrayList<UserInfo>
- type: ConversationType
+ ConversationMetadata(conversationId: long, p: ArrayList<UserInfo>, hp: ArrayList<UserInfo>, t: ConversationType)
+ getConversationId(): long
+ getParticipants(): ArrayList<UserInfo>
+ getHistoricalParticipants(): ArrayList<UserInfo>
+ getType(): ConversationType

class AdminConversationResult implements ResponsePayload
...
class Message implements ResponsePayload
- text: String
- sequenceNumber: long
- timestamp: Date
- senderId: String
- conversationId: long
+ Message(t: String, sn: long, time: Date, senderID: String, conversationId: long)
+ getText(): String
+ getConversationId(): long
+ getSenderId(): String
...
class ReadMessagesUpdated implements ResponsePayload
- updatedUserInfo: UserInfo
+ ReadMessagesUpdated(updatedUserInfo: UserInfo)
...
- directoryUserInfoList: ArrayList<UserInfo>
+ LoginResult(r: LoginStatus)
+ LoginResult(r: LoginStatus, ui: UserInfo, cl: ArrayList<Conversation>, directoryUsers: ArrayList<UserInfo>)
+ getLoginStatus(): LoginStatus
+ getUserInfo(): UserInfo
+ getConversationList(): ArrayList<Conversation>
+ getDirectoryUserInfoList(): ArrayList<UserInfo>

class LeaveResult implements ResponsePayload
- leftConversationID: long
+ LeaveResult(leftConversationID: long)
+ getLeftConversationID(): long
```
